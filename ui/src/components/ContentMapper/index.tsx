...
const ContentMapper = forwardRef(({ handleStepChange }: contentMapperProps, ref: React.ForwardedRef<ContentTypeSaveHandles>) => {
  ...
  const [isLoading, setIsLoading] = useState(false); // Changed from newMigrationData?.isprojectMapped
  ...
  const [contentTypes, setContentTypes] = useState<ContentType[]>([]); // Initialize as empty array
          ...
          if (contentTypeSchema?.every((item) => value?.value?.uid !== item?.uid)) {
            ...
            item?.schema?.forEach((schemaItem) => {
              ...
              if (value?.value?.uid === schemaItem?.uid && value?.label === `${item?.display_name} > ${schemaItem?.display_name}`) {
                ...
                (schema) => schema?.uid === existingField[id]?.value?.uid) && existingField[id]?.value?.data_type !== 'group' && existingField[id]?.label?.includes?.(item?.display_name)) {
        ...
        for (const item of array) {
          ...
          if (item?.id === data?.id) {
            for (const key of existingField[groupArray?.[0]?.backupFieldUid]?.value?.schema || []) {
              const id = key;
              if (checkConditions(fieldTypeToMatch, id, item)) {
                OptionsForRow.push(getMatchingOption(id, true, `${updatedDisplayName} > ${id?.display_name}` || '', `${uid}.${id?.uid}`));
              }

              // Recursively process nested groups
              if (id?.data_type === 'group') {
                processSchema(id, data, array, groupArray, OptionsForRow, fieldsOfContentstack, updatedDisplayName, uid);
              }
            }
          }
        }
  ...
  const handleFetchContentType = async () => {
    ...
    const contentFieldKey = Object.keys(contentTypeMapped)
      .find(id => contentTypeMapped[id] === otherContentType?.label);
                ...
                <div className='d-flex align-items-center'>
                  ...
                  {showFilter && (
                    <div className='filter-wrapper' ref={filterRef}>
                      <ul>
                        {Object.keys(CONTENT_MAPPING_STATUS)?.map?.((id, item) => {
                          return (

                            <li key={item}>
                              <button
                                className='list-button'
                                onClick={(e) => handleContentTypeFilter(CONTENT_MAPPING_STATUS[id], e)}
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter') {
                                    handleContentTypeFilter(CONTENT_MAPPING_STATUS[id], e);
                                  }
                                }}
                              >
                                {CONTENT_MAPPING_STATUS[id] && <span className={`${activeFilter === CONTENT_MAPPING_STATUS[id] ? 'filter-status filterButton-color ' : 'filter-status'}`}>{CONTENT_MAPPING_STATUS[id]}</span>}
                                {STATUS_ICON_Mapping[id] && <Icon size="small" icon={STATUS_ICON_Mapping[id]} className={STATUS_ICON_Mapping[id] === 'CheckedCircle' ? 'mapped-icon' : ''} />}
                              </button>
                            </li>
                          )
                        })}
