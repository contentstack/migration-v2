...
const ContentMapper = forwardRef(
  ({ handleStepChange }: contentMapperProps, ref: React.ForwardedRef<ContentTypeSaveHandles>) => {
    /** ALL CONTEXT HERE */
    ...
    const [otherContentType, setOtherContentType] = useState<FieldTypes>({
      label:
        ...
        `Select ${isContentType ? 'Content Type' : 'Global Field'} from Destination Stack`,
      value:
        contentTypeMapped?.[otherCmsTitle] ??
        `Select ${isContentType ? 'Content Type' : 'Global Field'} from Destination Stack`
    });
    ...
    useEffect(() => {
      if (existingField && !isCsCTypeUpdated) {
        ...
        if (
          newMigrationData?.content_mapping?.content_type_mapping?.[otherCmsTitle] !==
    ...
    const fetchFields = async (contentTypeId: string, searchText: string) => {
      try {
        ...
        const { data } = await getFieldMapping(
          contentTypeId || '',
          0,
          1000,
          searchText || '',
        ...
        const validTableData = data?.fieldMapping?.filter(
          (field: FieldMapType) => field?.otherCmsType !== undefined
        );
    ...
    const fetchEntries = async (contentTypeId: string) => {
      try {
        const { data } = await getEntryMapping(
          contentTypeId || '',
          0,
          1000,
          searchText || '',
    ...
    const openContentType = (i: number) => {
      ...
      const mappedContentType = contentModels?.find(
        (item) =>
          item?.uid === newMigrationData?.content_mapping?.content_type_mapping?.[otherTitle]
      );
      ...
      setOtherContentType({
        label:
          mappedContentType?.title ??
          `Select ${filteredContentTypes?.[i]?.type === 'content_type' ? 'Content Type' : 'Global Field'} from Destination Stack`,
        value:
          mappedContentType?.title ??
          `Select ${filteredContentTypes?.[i]?.type === 'content_type' ? 'Content Type' : 'Global Field'} from Destination Stack`
      });
    ...
    const updateFieldSettings = (
      ...
      rowContentstackFieldUid: string
    ) => {
    ...
    const accessorCall = (data: FieldMapType) => {
      ...
      const childrenCount = isModularBlockChild
        ? tableData?.filter((item) => item?.uid?.startsWith(data?.uid + '.'))?.length || 0
        : 0;
      return (
        <Tooltip content={tooltipContent} position="top">
          <div
            className={`table-row-content ${nestingLevel > 0 ? getNestingClass(nestingLevel) : ''}`}
            style={
              isModularBlock || isModularBlockChild || isGroup ? { borderLeft: '0 none' } : {}
            }>
            <div className="d-flex align-items-center">
              ...
              {data?.contentstackFieldType === 'modular_blocks' && (
                <Icon icon="ModularBlocks" size="small" className="mr-8 mt-1" />
              )}

              {isModularBlockChild && (
                <Icon icon="ParentModularBlockIcon" size="small" className="mr-8 mt-1" />
              )}
              {isGroup && <Icon icon="Group" size="small" className="mr-8 mt-1" />}

              <div
                className={`${data?.backupFieldType === 'text' || data?.backupFieldType === 'url' ? `cms-field w-auto` : `cms-field`}`}>
                {cleanFieldName}
              </div>
            ...
            <InstructionText>
              <div
                className={`${isModularBlock || isModularBlockChild || isGroup ? 'instruction-text-indent' : ''}`}>
                Type: {data?.otherCmsType}
                <br />
                <div className="uid-line">
                  <span className="uid-label">UID:</span>
                  <span className="uid-text">{data?.uid}</span>
                </div>
              </div>
            </InstructionText>
          </div>
    ...
    const getParentId = (uid: string) => {
      return (
        tableData?.find(
          (i) =>
            i?.uid?.toLowerCase() === uid?.toLowerCase() &&
            i?.backupFieldType?.toLowerCase() === 'group'
        )?.id ?? ''
      );
    };
    ...
    const modifiedObj = (obj: FieldMapType) => {
      ...
      return {
        ...
        backupFieldType,
        uid,
        parentId: excludeArr?.includes?.(backupFieldType?.toLowerCase())
          ? ''
          : getParentId(uid?.split('.')[0]?.toLowerCase()),
        _canSelect
      };
    };
    ...
    const cascadeUncheckParents = (
      ...
      const parentItem = data?.find(
        (item) => item?.uid?.toLowerCase() === parentUid?.toLowerCase()
      );
    ...
    const isLeafNode = (item: FieldMapType): boolean => {
      ...
      return type !== 'group' && type !== 'modular_blocks' && type !== 'modular_blocks_child';
    };
    ...
    const SelectAccessor = (data: FieldMapType) => {
      ...
      const fieldLabel =
        data?.contentstackFieldType === 'url' || data?.contentstackFieldType === 'group'
          ? data?.contentstackFieldType
          : initialOption?.label;
            ...
            <Select
              ...
              isDisabled={
                !(
                  ...
                  data?.contentstackFieldType === 'multi_line_text' ||
                  data?.contentstackFieldType === 'html' ||
                  data?.contentstackFieldType === 'json'
                ) ||
    ...
    const handleFieldChange = (
      ...
      const groupArray = nestedList?.filter((item) => item?.child?.some((e) => e?.id));

      if (
        groupArray?.[0]?.child &&
        previousSelectedValue !== selectedValue?.label &&
        groupArray?.[0]?.uid === rowIndex
      ) {
      ...
      setSelectedOptions((prevSelected) => {
        const newSelectedOptions = prevSelected?.filter((item) => item !== previousSelectedValue);
      ...
      const updatedRows: FieldMapType[] = tableData.map((row) => {
        if (row?.uid === rowIndex && row?.contentstackFieldUid === backupFieldUid) {
          return {
            ...
            contentstackFieldType: selectedValue?.value?.field_metadata?.allow_rich_text
              ? 'html'
              : row?.contentstackFieldType,
            advanced: {
    ...
    const generateSourceGroupSchema = (schema: FieldMapType[]) => {
      ...
      schema?.forEach((field: FieldMapType) => {
        ...
        // Handle modular blocks child
          ...
          field?.uid?.startsWith(modularBlockId + '.')
        ) {
        ...
        // Handle fields within modular block child
          ...
          if (!parentBlock) {
            ...
            if (!childBlock) {
              ...
              console.warn(`Child block exists but missing 'child' property: ${childBlock?.uid}`);
            } else {
    ...
    function getMatchingOption(
      ...
      return matchFound
        ? { label, value, isDisabled: selectedOptions?.includes?.(label), uid: uid }
        : {};
    }

    //utility function to map the source cms field type to content type field type
    function checkConditions(
      fieldTypeToMatch: string | string[],
      value: ContentTypesSchema,
      data: FieldMapType
    ) {
    ...
    const processSchema = (
        ...
        if (value?.blocks && Array.isArray(value.blocks)) {
          for (const block of value.blocks) {
            ...
            if (data?.backupFieldType === 'modular_blocks_child') {
              ...
              OptionsForRow.push(getMatchingOption(blockOption, true, blockDisplayName, blockUid));
            }
            ...
            if (isFieldInsideChildBlock && block?.schema && Array.isArray(block.schema)) {
              ...
              const modularBlockArray = nestedList?.filter(
                (item) =>
              ...
              const mappedChildBlockTitle = existingChildBlockMapping?.label
                ?.split(' > ')
                ?.pop()
                ?.trim();
              ...
              if (mappedChildBlockTitle === blockTitle) {
                for (const blockField of block.schema) {
                  ...
                  if (checkConditions(fieldTypeToMatch, blockField, data)) {
                    ...
                    OptionsForRow.push(
                      getMatchingOption(blockField, true, fieldDisplayName, fieldUid)
                    );
                  }
      ...
      if (value?.data_type === 'group') {
        ...
        const fieldTypeToMatch = Fields[data?.backupFieldType as keyof Mapping]?.type;
        if (
          !array.some((item: FieldMapType) => item?.id === data?.id) &&
          checkConditions(fieldTypeToMatch, value, data)
        ) {
          OptionsForRow.push(getMatchingOption(value, true, updatedDisplayName || '', uid ?? ''));
        }
    ...
    const SelectAccessorOfColumn = (data: FieldMapType) => {
      ...
      if (contentTypeSchema && validateArray(contentTypeSchema)) {
        ...
        for (const value of contentTypeSchema) {
          if (
            data?.uid === value?.uid &&
            data?.backupFieldType === value?.data_type &&
            fieldTypeToMatch
          ) {
        ...
        if (OptionsForRow?.length === 0) {
          for (const value of contentTypeSchema) {
            ...
            // Process leaf fields
              ...
              !array?.some((item) => item?.id === data?.id) &&
              checkConditions(fieldTypeToMatch, value, data)
            ) {
              OptionsForRow.push(
                getMatchingOption(value, true, value?.display_name || '', value?.uid ?? '')
              );
            }
          }
        }
      ...
      if (
        ...
        (OptionsForRow?.[0]?.value?.uid === 'url' ||
          OptionsForRow?.[0]?.value?.uid === 'title' ||
          OptionsForRow?.[0]?.value?.data_type === 'group' ||
          OptionsForRow?.[0]?.value?.data_type === 'reference')
      ) {
        updatedRows = updatedRows.map((row: FieldMapType) => {
        ...
        const hasMatchingEntry = Object.values(updatedExstingField)?.some((entry) => {
      ...
      const OptionValue: FieldTypes =
        OptionsForRow?.length === 1 &&
        (existingField[data?.backupFieldUid] || updatedExstingField[data?.backupFieldUid]) &&
        (OptionsForRow?.[0]?.value?.uid === 'url' ||
          OptionsForRow?.[0]?.value?.uid === 'title' ||
          OptionsForRow?.[0]?.value?.data_type === 'group' ||
          OptionsForRow?.[0]?.value?.data_type === 'reference')
          ...
          : OptionsForRow?.length === 0 ||
              (OptionsForRow?.length > 0 &&
                OptionsForRow?.every((item) => item?.isDisabled) &&
                (!existingField[data?.backupFieldUid]?.label ||
                  !updatedExstingField[data?.backupFieldUid]?.label))
            ? {
                label: Fields[data?.contentstackFieldType]?.label ?? 'No Option',
                value: Fields[data?.contentstackFieldType]?.label ?? 'No Option',
                isDisabled:
      ...
      const adjustedOptions: OptionsType[] | OptionsType =
        OptionsForRow.length === 0 && !contentTypeSchema
          ? option
          : OptionsForRow?.length > 0 &&
              OptionsForRow?.every((item) => item?.isDisabled) &&
              OptionValue?.label === Fields[data?.contentstackFieldType]?.label
            ? []
      ...
      const isTypeMatch = checkConditions(
        Fields[data?.contentstackFieldType]?.type?.toLowerCase(),
        existingField[data?.backupFieldUid]?.value,
        data
      );
        ...
        <div className="table-row">
          <div className="select">
            <Select
              ...
              isClearable={
                isTypeMatch &&
                selectedOptions?.includes?.(existingField?.[data?.backupFieldUid]?.label ?? '')
              }
          ...
          {(!OptionValue?.isDisabled ||
            ...
            <div className="advanced-setting-button">
              <Tooltip
                content="Advanced properties"
                position="top"
                disabled={data?.otherCmsField === 'title' || data?.otherCmsField === 'url'}>
                <Button
                  buttonType="light"
                  disabled={
                    (contentTypeSchema && existingField[data?.backupFieldUid]) ||
                    newMigrationData?.project_current_step > 4
                  }
                  onClick={() => {
                  ...
                  <Icon version={'v2'} icon="Sliders" size="small" />
                </Button>
              </Tooltip>
            </div>
          )}
        ...
        try {
          ...
          if (data?.status == 200) {
            ...
            const savedCT = filteredContentTypes?.map?.((ct) =>
              ct?.id === data?.data?.updatedContentType?.id
                ? { ...ct, status: data?.data?.updatedContentType?.status }
                : ct
            );
            let filteredCT = savedCT;
            if (!isEmptyString(activeFilter) && activeFilter !== 'All') {
              filteredCT = savedCT?.filter(
                (ct) => CONTENT_MAPPING_STATUS?.[ct?.status] === activeFilter
              );
            }
            const savedContentTypes = contentTypes?.map?.((ct) =>
              ct?.id === selectedContentType?.id
                ? { ...ct, status: data?.data?.updatedContentType?.status }
                : ct
            );
            ...
            const FailedCT = filteredContentTypes?.map?.((ct) =>
              ct?.id === selectedContentType?.id
                ? { ...ct, status: selectedContentType?.status }
                : ct
            );
    ...
    const handleFetchContentType = debounce(async () => {
      ...
      if (isContentType) {
        try {
          const { data, status } = await getExistingContentTypes(
            projectId,
            otherContentType?.id ?? ''
          );
          if (status == 201 && data?.contentTypes?.length > 0) {
            ...
            otherContentType?.id &&
              otherContentType?.label !== data?.selectedContentType?.title &&
              data?.selectedContentType?.title &&
              setOtherContentType({
          ...
          if (
            otherContentType?.id &&
            data?.contentTypes?.every((item: FieldMapType) => item?.uid !== otherContentType?.id)
          ) {
        ...
        try {
          const { data, status } = await getExistingGlobalFields(
            projectId,
            otherContentType?.id ?? ''
          );
          ...
          if (status == 201 && data?.globalFields?.length > 0) {
            ...
            otherContentType?.id &&
              otherContentType?.label !== data?.selectedGlobalField?.title &&
              data?.selectedGlobalField?.title &&
              setOtherContentType({
          ...
          if (
            otherContentType?.id &&
            data?.globalFields?.every((item: FieldMapType) => item?.uid !== otherContentType?.id)
          ) {
      ...
      const contentField = contentModels?.find(
        (item: ContentTypeList) => item?.title === otherContentType?.label
      );
      const contentFieldKey = Object.keys(contentTypeMapped).find(
        (key) => contentTypeMapped[key] === otherContentType?.label
      );
    ...
    const adjustedOption = options?.map?.((option) => ({
      ...option,
      isDisabled:
        filteredContentTypes?.some?.((ct) => ct?.contentstackUid === option?.id) ||
        (contentTypeMapped && Object.values(contentTypeMapped)?.includes?.(option?.id))
    }));
      ...
      <div className="step-container">
        {contentTypes?.length > 0 || tableData?.length > 0 ? (
          <div className="d-flex flex-wrap table-container">
            ...
            <div className="content-types-list-wrapper">
              <div className="content-types-list-header d-flex align-items-center justify-content-between">
                {contentTypesHeading && (
                  <h2>{`${contentTypesHeading} (${contentTypes && count})`}</h2>
                )}
              </div>
                        ...
                        {Object.keys(CONTENT_MAPPING_STATUS)?.map?.((key, keyInd) => (
                          <li key={`${keyInd?.toString()}`}>
                            <button
                              ...
                              {CONTENT_MAPPING_STATUS[key] && (
                                <span
                                  className={`${activeFilter === CONTENT_MAPPING_STATUS[key] ? 'filter-status filterButton-color' : 'filter-status'}`}>
                                  {CONTENT_MAPPING_STATUS[key]}
                                </span>
                              )}
                        ...
                        <li
                          ...
                          <button
                            ...
                            onKeyDown={(e) => {
                              if (
                                e.key === 'Enter' &&
                                otherCmsUid !== filteredContentTypes[index]?.otherCmsUid
                              ) {
                                handleOpenContentType(index);
                              }
                            }}>
                            <div className="cms-title">
                              ...
                              {content?.otherCmsTitle && (
                                <span title={content?.otherCmsTitle}>{content?.otherCmsTitle}</span>
                              )}
                            </div>
                          </button>
                          <div className="d-flex align-items-center ct-options">
                            <span>
                              {icon && (
                                <Tooltip
                                  content={CONTENT_MAPPING_STATUS[content?.status]}
                                  position="bottom">
                                  <Icon
                                    size="small"
                                    icon={icon}
                                    className={icon === 'CheckedCircle' ? 'mapped-icon' : ''}
                                  />
                                </Tooltip>
                              )}
                            </span>
                            <span className="ml-10">
                              <Tooltip content="Schema Preview" position="bottom">
                                <button
                                  className="list-button schema-preview"
                                  aria-label="schemaPreview"
                                  onClick={() =>
                                    handleSchemaPreview(content?.otherCmsTitle, content?.id ?? '')
                                  }>
                                  {SCHEMA_PREVIEW}
                                </button>
                              </Tooltip>
                          ...
                          <div className="d-flex align-items-center">
                            {!isNewStack && (
                              <>
                                ...
                                <Tooltip
                                  content={'Fetch content types from destination stack'}
                                  position="top">
                                  <Button
                                    className="icon-padding"
                                    buttonType="light"
                                    icon={onlyIcon ? 'v2-FetchTemplate' : ''}
                                    version="v2"
                                    onlyIcon={true}
                                    onlyIconHoverColor={'primary'}
                                    size="small"
                                    onClick={handleFetchContentType}
                                    disabled={isResetFetch}></Button>
                                </Tooltip>
                              </>
                            )}
          ...
          <EmptyState
            ...
            description={<div className="empty_search_description">{modalProps?.body}</div>}
