...
const ContentMapper = forwardRef(
  ({ handleStepChange }: contentMapperProps, ref: React.ForwardedRef<ContentTypeSaveHandles>) => {
    ...
    const openContentType = (i: number) => {
      ...
      setOtherContentType({
        label:
          ...
          `Select ${
            filteredContentTypes?.[i]?.type === 'content_type' ? 'Content Type' : 'Global Field'
          } from existing stack`,
        value:
          mappedContentType?.title ??
          `Select ${
    ...
    const accessorCall = (data: FieldMapType) => {
      return (
        <div>
          <Tooltip content={data?.otherCmsField} position="bottom">
    ...
    const handleSelectedEntries = (singleSelectedRowIds: string[]) => {
      ...
      if (latestRow?.backupFieldType?.toLowerCase() === 'group' && latestRow?.parentId === '') {
        ...
        const childItems = tableData?.filter((entry) =>
          entry?.uid?.toLowerCase()?.startsWith(groupUid + '.')
        );
        ...
        latestRow?.parentId &&
        !['title', 'url']?.includes?.(latestRow?.uid?.toLowerCase())
      ) {
        ...
        const childItems = tableData?.filter((entry) =>
          entry?.uid?.toLowerCase()?.startsWith(groupItem?.uid + '.')
        );
      ...
      return (
        <div className="table-row">
          ...
          {!(
            ...
            >
              <Button buttonType="light" disabled={newMigrationData?.project_current_step > 4}>
                <Icon
                  version="v2"
                  icon="Sliders"
                  size="small"
    ...
    const SelectAccessorOfColumn = (data: FieldMapType) => {
      ...
      if (contentTypeSchema && validateArray(contentTypeSchema)) {
        ...
        for (const value of contentTypeSchema) {
          if (
            data?.uid === value?.uid ||
            (data?.uid === value?.uid && data?.otherCmsType === value?.data_type)
          ) {
      ...
      const OptionValue: FieldTypes =
        ...
        (OptionsForRow?.[0]?.value?.uid === 'url' ||
          ...
          : OptionsForRow?.length === 0 ||
            ...
            (OptionsForRow?.length > 0 && data?.contentstackFieldType === 'dropdown')
          ? {
              label: Fields[data?.contentstackFieldType]?.label ?? 'No Option',
              value: Fields[data?.contentstackFieldType]?.label ?? 'No Option',
              isDisabled:
      ...
      return (
        <div className="table-row">
          <div className="select">
            <Select
              value={
                OptionsForRow?.length === 0 || existingField?.[data?.uid]?.label === undefined
                  ? OptionValue
                  : existingField[data?.uid]
              }
              onChange={(selectedOption: FieldTypes) => {
                if (OptionsForRow?.length === 0) {
                  handleValueChange(selectedOption, data?.uid);
            ...
            <div className="advanced-setting-button">
              ...
              >
                <Button
                  buttonType="light"
                  disabled={
                    (contentTypeSchema && existingField[data?.uid]) ||
                    newMigrationData?.project_current_step > 4
                  }
                >
                  <Icon
        ...
        {contentTypes?.length > 0 || tableData?.length > 0 ? (
          <div className="d-flex flex-wrap table-container">
            ...
            <div className="content-types-list-wrapper">
              <div className="content-types-list-header d-flex align-items-center">
                {contentTypesHeading && <h2>{`${contentTypesHeading} (${count})`}</h2>}
                {/* {contentTypes && (<strong>{`(${count})`}</strong>)} */}
              </div>
            ...
            <div className="content-types-fields-wrapper">
              <div className="table-wrapper">
                ...
                />
                <div className="my-2 mx-3 px-1 py-1 d-flex justify-content-between align-items-center">
                  <div><strong>Total Fields: {totalCounts}</strong></div>
                  <Button
