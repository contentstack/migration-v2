...
// Interfaces
import { StackOption, FileOption, TableDataItem, FilterOption } from './auditLogs.interface';
import './index.scss';
import { auditLogsConstants } from '../../utilities/constants';
import AuditFilterModal from '../AuditFilterModal';

// const renderCell = (value: string | null | undefined) => <div>{value ?? '-'}</div>;
const renderCell = (value: string | null | undefined, flag = 0) => (
  flag ? <Tooltip
  content={value ? value : 'No value provided'}
  position="bottom"
  variantType="dark"
  showArrow={true}>
...
const AuditLogs: React.FC = () => {
  ...
  const isMigDone = useSelector(
    (state: RootState) =>
      state?.migration?.newMigrationData?.migration_execution?.migrationCompleted
  );
  const label1 = useSelector(
    (state: RootState) => state?.migration?.newMigrationData?.stackDetails?.label
  );
  const value1 = useSelector(
    (state: RootState) => state?.migration?.newMigrationData?.stackDetails?.value
  );
  useEffect(() => {
    if (stacks && stacks?.length > 0) {
      const formattedOptions: StackOption[] = stacks.map((stack) => ({
        label: stack?.stackName || '',
        value: stack?.stackUid || '',
        stackName: stack?.stackName,
        stackUid: stack?.stackUid,
        isMigrated: stack?.isMigrated
      }));
  ...
  return (
    <div className="table-height">
      <InfiniteScrollTable
        ...
        data={tableData}
        columns={
          dropDownOptions == 'content-types' || dropDownOptions == 'global-fields'
            ? contentTypeHeader
            : entryHeader
        }
        uniqueKey={'id'}
        ...
        customEmptyState={
          <EmptyState
            heading={searchText === '' ? auditLogsConstants?.noLogs : auditLogsConstants?.noResult}
            description={
              searchText === ''
