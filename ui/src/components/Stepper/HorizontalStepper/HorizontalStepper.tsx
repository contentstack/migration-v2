...
const HorizontalStepper = forwardRef(
  (props: stepperProps, ref: React.ForwardedRef<HorizontalStepperHandles>) => {
    ...
    const setTabStep = (idx: number) => {
      if (
        (stepsCompleted?.includes(idx) || stepsCompleted?.length === idx) &&
        ...
        {steps?.map(({ id, title }, idx: number) => {
          ...
          const disableClass =
            !stepsCompleted.includes(idx) && idx !== showStep && !stepsCompleted?.includes(idx - 1)
              ...
              : '';
          const disableStep =
            newMigrationData?.isprojectMapped && stepsCompleted.includes(idx) && idx !== showStep
              ? 'disableEvents'
              : '';
          ...
          const disableMapper =
            ...
            idx === 2 &&
            newMigrationData?.test_migration?.isMigrationStarted &&
            !newMigrationData?.test_migration?.isMigrationComplete
              ? 'disableEvents'
              : '';
          ...
          return (
            <React.Fragment key={id}>
              ...
              {idx < steps?.length - 1 && (
                <div className={completedClass ? 'connector' : 'disabled-connector'}></div>
              )}
            </React.Fragment>
          );
        })}
