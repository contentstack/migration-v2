 ...
 */
const projectBase = (projectId: string) => `${API_VERSION_V3}/project/${projectId}`;

export const wizardApi = {
  /** The persisted `source` document — app-bar name (TR-3), Source completion (TR-8). */
  getSource: (projectId: string) => apiClient.get(`${projectBase(projectId)}/source`),

  /** The persisted `destination` document — Destination completion (TR-8). */
  getDestination: (projectId: string) =>
    apiClient.get(`${projectBase(projectId)}/destination`),
};
