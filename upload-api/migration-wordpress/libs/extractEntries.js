...
/** All source entry keys use posts_* for every content type (matches wordpress.service export). */
...
const extractEntries = async (filePath, contentTypeData = []) => {
  try {
    ...
    if (authorData) {
      const authorEntries = normalizeArray(authorData).map((author) => ({
        'wp:post_type': 'author',
    ...
    if (termData) {
      const termEntries = normalizeArray(termData).map((term) => ({
        'wp:post_type': 'terms',
