...
import { extractContentTypes, extractEntries, extractLocale } from 'migration-wordpress';
...
const createWordpressMapper = async (filePath: string = '',projectId: string | string[],app_token: string | string[],affix: string | string[],config: any
  ...
  try {
    ...
    const extractedContentTypes: any = await extractContentTypes(affix as string, filePath, config);
    const contentTypeData: any = await extractEntries(filePath, extractedContentTypes);
