...
const loadTaxonomySchema = async (dbConfig = null) => {
  try {
    // Use process.cwd() to ensure we read from the upload-api root directory
    // This matches the behavior of extractTaxonomy and createInitialMapper
...
/**
 * Helper function to check if reference field should be skipped entirely
 * Returns true if the field should be skipped (only references profile or empty after filtering)
 * @param {Array} referenceFields - Array of content type UIDs
 * @returns {boolean} True if field should be skipped
 */
function shouldSkipReferenceField(referenceFields) {
  if (!Array.isArray(referenceFields) || referenceFields.length === 0) {
    return true; // Skip if no references
  }

  // Filter out profile
  const filtered = filterOutProfile(referenceFields);
 ...
 */
const contentTypeMapper = async (
  data,
  contentTypes,
  prefix,
  dbConfig = null,
  actualTaxonomyUsage = new Set()
) => {
  ...
  const schemaArray = data.reduce((acc, item) => {
    ...
    switch (item.type) {
      ...
      case 'text_long': {
        ...
        // Fix: backupFieldType should match contentstackFieldType for correct reset behavior
      ...
      case 'string': {
        ...
        const referenceFields = filterOutProfile(availableContentTypes.slice(0, 10));
        // Fix: backupFieldType should match contentstackFieldType for correct reset behavior
        acc.push(createFieldObject(item, 'single_line_text', 'single_line_text', referenceFields));
        break;
      }
      case 'telephone': {
      ...
      case 'taxonomy_term_reference': {
        ...
        if (taxonomySchema && taxonomySchema.length > 0) {
          ...
          // NOTE: Removed fallback that added ALL taxonomies - only include those with actual data
        } else {
