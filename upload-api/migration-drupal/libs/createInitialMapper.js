...
const getActualTaxonomyUsage = async (connection, contentType) => {
  ...
  try {
    ...
    for (const fieldTable of fieldTables) {
      ...
      // Skip known image/file fields by name pattern to avoid ID collision false positives
      // (image file IDs can coincidentally match taxonomy term IDs)
      if (
...
const createInitialMapper = async (systemConfig, prefix) => {
  try {
    // Use process.cwd() to ensure folder is created in the upload-api root directory
    // config.data is "./drupalMigrationData", resolve it relative to process.cwd()
    const baseDir = path.resolve(process.cwd(), config.data);
    const drupalFolderPath = path.join(baseDir, config.drupal.drupal);

    // Create fresh directory (cleanup is now handled at service level)
    // recursive: true will create both drupalMigrationData and drupalSchema if they don't exist
