...
export const analyzeFieldTypes = async (
  ...
  projectId: string
): Promise<{ taxonomyFields: TaxonomyFieldMapping; referenceFields: ReferenceFieldMapping; assetFields: AssetFieldMapping }> => {
  ...
  try {
    ...
    const taxonomyFieldMapping: TaxonomyFieldMapping = {};
    const referenceFieldMapping: ReferenceFieldMapping = {};
    const assetFieldMapping: AssetFieldMapping = {};
    ...
    for (const fieldConfig of fieldConfigs) {
      try {
        ...
        if (fieldData && fieldData.field_name && fieldData.bundle) {
          ...
          // Initialize content type mappings if not exists
          if (!taxonomyFieldMapping[fieldInfo.content_types]) {
            taxonomyFieldMapping[fieldInfo.content_types] = {};
          }
          if (!referenceFieldMapping[fieldInfo.content_types]) {
            referenceFieldMapping[fieldInfo.content_types] = {};
          }
          if (!assetFieldMapping[fieldInfo.content_types]) {
            assetFieldMapping[fieldInfo.content_types] = {};
          }
          
          // Check if this is a taxonomy reference field
          const isTaxonomyField = 
            // Check handler for taxonomy references
            (fieldInfo.content_handler && fieldInfo.content_handler.includes('taxonomy_term')) ||
            ...
            (fieldInfo.field_type === 'entity_reference' && fieldInfo.target_type === 'taxonomy_term') ||
            ...
            (fieldInfo.handler_settings?.target_bundles && 
             Object.keys(fieldInfo.handler_settings.target_bundles).some(bundle => 
               fieldInfo.target_type === 'taxonomy_term'));
          ...
          const isReferenceField = 
            ...
            (fieldInfo.content_handler && fieldInfo.content_handler.includes('node')) ||
          ...
          if (isTaxonomyField) {
            ...
            if (fieldInfo.handler_settings?.target_bundles) {
              const vocabularies = Object.keys(fieldInfo.handler_settings.target_bundles);
              vocabulary = vocabularies.length === 1 ? vocabularies[0] : vocabularies.join(',');
            }
            
            taxonomyFieldMapping[fieldInfo.content_types][fieldInfo.field_name] = {
              vocabulary,
            ...
            await customLogger(projectId, destination_stack_id, 'info', taxonomyMessage);
          } else if (isReferenceField) {
            referenceFieldCount++;
            
            referenceFieldMapping[fieldInfo.content_types][fieldInfo.field_name] = {
            ...
            const referenceMessage = getLogMessage(
              srcFunc,
              `Found reference field: ${fieldInfo.content_types}.${fieldInfo.field_name} → target_type: ${fieldInfo.target_type || 'node'}`,
              {}
            );
            await customLogger(projectId, destination_stack_id, 'info', referenceMessage);
          }
          ...
          const isAssetField = 
            ...
            (fieldInfo.field_type === 'entity_reference' && fieldInfo.target_type === 'file');
          ...
          if (isAssetField) {
            ...
            const uploadLocation = fieldData?.settings?.file_directory || 
              fieldData?.settings?.uri_scheme || 'public://';
            const maxFilesize = fieldData?.settings?.max_filesize || 
              fieldData?.settings?.file_size || '';
            
            assetFieldMapping[fieldInfo.content_types][fieldInfo.field_name] = {
              field_type: fieldInfo.field_type,
            ...
            const assetMessage = getLogMessage(
              srcFunc,
              `Found asset field: ${fieldInfo.content_types}.${fieldInfo.field_name} → type: ${fieldInfo.field_type}, extensions: [${fileExtensions.join(', ')}]`,
              {}
            );
            await customLogger(projectId, destination_stack_id, 'info', assetMessage);
          }
        }
        ...
        await customLogger(projectId, destination_stack_id, 'warn', parseMessage);
      }
    }
...
export const isTaxonomyField = (
  ...
  return !!(taxonomyMapping[contentType] && taxonomyMapping[contentType][fieldName]);
};
...
export const isReferenceField = (
  ...
  return !!(referenceMapping[contentType] && referenceMapping[contentType][fieldName]);
};
...
export const transformTaxonomyValue = async (
  ...
  const fieldInfo = getTaxonomyFieldInfo(fieldName, contentType, taxonomyMapping);
  ...
  if (typeof value === 'number' || (typeof value === 'string' && /^\d+$/.test(value))) {
    ...
    try {
      ...
      const vocabularies = fieldInfo.vocabulary ? fieldInfo.vocabulary.split(',') : ['unknown'];
      ...
      for (const vocabulary of vocabularies) {
        try {
          ...
          const taxonomyFilePath = path.join(taxonomyBasePath, `${vocabulary}.json`);
          
          if (fs.existsSync(taxonomyFilePath)) {
            const taxonomyContent = JSON.parse(fs.readFileSync(taxonomyFilePath, 'utf8'));
      ...
      if (fs.existsSync(taxonomyBasePath)) {
        const taxonomyFiles = fs.readdirSync(taxonomyBasePath)
          .filter(file => file.endsWith('.json') && file !== 'taxonomies.json');
        
        for (const file of taxonomyFiles) {
          try {
            const taxonomyContent = JSON.parse(fs.readFileSync(path.join(taxonomyBasePath, file), 'utf8'));
