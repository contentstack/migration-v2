    ...
    try {
      ...
      const apiField = convertToSchemaFormate({
        field: {
          ...
          advanced: {
            ...
            // Then override with UI selections from savedMapping (database)
            mandatory:
              savedMapping?.advanced?.mandatory ??
              uploadField.advanced?.mandatory ??
              false,
            multiple:
              savedMapping?.advanced?.multiple ??
              uploadField.advanced?.multiple ??
              false,
            unique:
              savedMapping?.advanced?.unique ??
              uploadField.advanced?.unique ??
              false,
            nonLocalizable:
              savedMapping?.advanced?.nonLocalizable ??
              uploadField.advanced?.non_localizable ??
              false,
            default_value:
              savedMapping?.advanced?.default_value ??
              uploadField.advanced?.default_value ??
              '',
            validationRegex:
              savedMapping?.advanced?.validationRegex ??
              uploadField.advanced?.format ??
              '',
            validationErrorMessage:
              savedMapping?.advanced?.validationErrorMessage ??
              uploadField.advanced?.error_message ??
              '',
            embedObjects:
              savedMapping?.advanced?.embedObjects ??
              uploadField.advanced?.embedObjects ??
              [],
            embedObject: savedMapping?.advanced?.embedObject ?? false,
          },
          // For reference fields, use UI selections first, then fall back to upload-api
          refrenceTo:
            savedMapping?.refrenceTo ||
      ...
      if (apiField) {
        ...
        if (fieldType === 'reference') {
          // Check both spellings: refrenceTo (legacy typo used by UI) and referenceTo (correct spelling)
          const uiReferences =
            savedMapping?.refrenceTo || savedMapping?.referenceTo || [];
          const hasUIReferences =
            Array.isArray(uiReferences) && uiReferences.length > 0;

          // Also check advanced.referenedItems which is where the UI stores reference selections
          const advancedReferences =
            savedMapping?.advanced?.referenedItems || [];
          const hasAdvancedReferences =
            Array.isArray(advancedReferences) && advancedReferences.length > 0;

          if (hasUIReferences || hasAdvancedReferences) {
            // MERGE: Combine old upload-api data with new UI selections (no duplicates)
            // Check both embedObjects AND reference_to from upload-api
            const oldReferences =
              uploadField.advanced?.embedObjects ||
              uploadField.advanced?.reference_to ||
              [];
        ...
        if (fieldType === 'taxonomy') {
          ...
          const uiTaxonomies =
            savedMapping?.refrenceTo || savedMapping?.referenceTo || [];
          const hasUITaxonomies =
            Array.isArray(uiTaxonomies) && uiTaxonomies.length > 0;

          // Also check advanced.referenedItems which is where the UI stores taxonomy selections
          const advancedTaxonomies =
            savedMapping?.advanced?.referenedItems || [];
          const hasAdvancedTaxonomies =
            Array.isArray(advancedTaxonomies) && advancedTaxonomies.length > 0;

          if (hasUITaxonomies || hasAdvancedTaxonomies) {
            ...
            apiField.taxonomies = mergedTaxonomyUIDs.map((taxUid: string) => ({
              ...
              multiple:
                savedMapping?.advanced?.multiple ??
                uploadField.advanced?.multiple !== false, // Default to true for taxonomies
              non_localizable:
                savedMapping?.advanced?.nonLocalizable ??
                uploadField.advanced?.non_localizable ??
                false,
