...
const processFieldByType = (
  ...
  switch (targetType) {
    ...
    case 'taxonomy': {
      // Taxonomy processing - return the value as-is (will be processed by consolidateTaxonomyFields)
      // The actual taxonomy lookup and transformation happens in the main entry processing loop
      // This case just ensures the value passes through without error
      if (Array.isArray(value)) {
        return value;
      }
      if (
        ...
        // Single taxonomy term ID - return as array for consistent processing
        return [Number(value)];
      }
      return value;
    }
    ...
    case 'link': {
      // Link field processing - ensure proper structure
