...
export const runCli = async (
  ...
  try {
    ...
    if (userData?.authtoken && stack_uid || userData?.access_token && stack_uid) {
      ...
      // project record rather than propagated here. Fires AFTER the completion message above
      // so the log reads in the order the UI (and MigrationLogViewer's own tests) expect:
      // "Migration Process Completed" first, then "Starting automatic post-migration
      // reconciliation ...".
      if (projectIndex > -1 && !isTest) {
        const proj: any = ProjectModelLowdb.data.projects[projectIndex];
    ...
    // without an explicit failure marker the UI spins forever with no way to retry. This
    // also resets isMigrationStarted directly, in addition to whatever migration.service.ts's
    // own catch does with the rethrow below — redundant but harmless (idempotent).
