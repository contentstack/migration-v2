...
## Post-run follow-ups (after the three phases closed)

- **Bug: a newly created stack didn't appear in the Stack dropdown.** `stackCreated` set `stackApiKey` but never appended the stack to the `stacks` array the dropdown builds its options from, so the control was bound to a value with no matching option and rendered blank. (The header/summary/Stack-contents card still showed it, because they fall back to `stackName` — which is why it looked half-working.) Fixed by appending `{ value, label }` in the reducer. TC_DEST_010 was **strengthened in place** to assert the new stack is actually selectable — it failed first (`expected [] to contain…`), then passed. The design prototype did this (`destStacks: [...st.destStacks, name]`); it was missed on the first pass, and no existing assertion covered "selectable", only "selected in state".
- **Migrated all Destination dropdowns to the shared `V3Select`.** `cs-source-selection` introduced a themed dropdown (native `<select>` renders its open list with OS chrome, ignoring the app theme). Five native selects remained here — 3 in `DestinationPanel`, 2 in `MappingRow` — so Destination would have shown the OS popup while Source showed the themed one. All five migrated; **0 native `<select>` left** in the feature. Verified visually: the Stack list now renders in-app with the violet selected-row highlight.
  - This changed the DOM contract (trigger `<button>` + `role="listbox"`/`role="option"` rendered only while open, `onChange(value)` instead of an event), so 10 tests had to drive the widget differently. **Every assertion's intent was preserved** — placeholder checks moved from a placeholder `<option>` to the trigger's text, and option checks now open the dropdown first and scope to the labelled listbox.
  - It also surfaced one **vacuous assertion**: TC_DEST_001 read `(trigger as HTMLSelectElement).value === ''`, which kept passing against a `<button>` purely because `HTMLButtonElement.value` defaults to `''`. Rewritten to assert the three triggers show their placeholders.
  - `V3Select` currently lives in `ui/v3/components/source/`. Importing it from `destination/` is v3-internal reuse (same rationale as reusing Source's endpoints), but it is now shared by both features and would be better under a common folder — not moved here, since that touches Source's imports.

- **Added a Master locale picker to the create-stack modal (new requirement).** A stack's master locale is fixed at creation and cannot be changed afterwards, so the modal is the only place it can be set. The modal now asks for **name → master locale → description**, and Create stays disabled until both name and locale are chosen. On success the chosen locale becomes the **destination side of the locked master-locale mapping row**, and the new stack's locale list is set to exactly that one locale (a brand-new stack has no others; any list left from a previously selected stack is replaced).
  - Backend: `createStack` now forwards `master_locale` to Contentstack (it previously sent only name/description, so stacks were silently getting Contentstack's default). New `GET /v3/destination/contentstack-locales` → `csManagement.listContentstackLocales`, which calls `/locales?include_all=true` — the full supported-locale set, **not** a stack's own locales, since the stack doesn't exist yet. Both mirror v2's `orgService.createStack` / `getLocales`.
  - `masterLocale` is **required in the UI but optional on the API** — matching v2, which also passes it through without validating. A caller omitting it gets Contentstack's default rather than a 400.
  - Covered by 3 new paired tests (slice / modal / thunk); the create-stack thunk tests were updated to seed a master locale, since the request no longer fires without one.
  - ⚠️ **This contradicts the frozen spec.** `feature.md` FR-1.4 says the modal captures "a required Stack name input and an optional Stack description", and §5 explicitly lists *"Full stack configuration at creation time"* as a **non-goal**, stating anything beyond name/description uses Contentstack's defaults. AC-7.1 says the same. The matrix has no row for a master-locale field, so TC_DEST_008/009 were extended rather than a new `TC_DEST_###` invented. **feature.md / prd.md / trd.md and the test-case matrix all need updating to match** — not done here, since those are upstream artifacts owned by the `feature-spec` / `prd-trd` / `test-case` skills.

- **Bug: `/locales` returns an OBJECT, not an array — the master-locale picker threw.** Contentstack returns that list keyed by locale code (`{ "en-us": {...} }`). `x ?? []` does not guard against it (the value is truthy), so it passed through and the next `.map` threw `TypeError: …map is not a function`, which surfaced through the v3 error middleware as a 500. Fixed with an `asArray()` normaliser (array → as-is, object → `Object.values`, else `[]`) applied to every list-shaped Contentstack read in this feature: `listContentstackLocales`, `listLocales`, and `getStackStats`'s content-types / global-fields / locales / branches. Reproduced by a test first — the failing assertion emitted the exact user-reported message — then fixed.
  - Related: **the failure was originally invisible.** `loadContentstackLocales` swallowed errors with a bare `catch {}`, leaving a silently empty dropdown that blocked stack creation with no explanation. It now tracks loading/error state, disables the picker while loading or errored, reports "Contentstack returned no locales." for a 200 with an unusable body, and shows the underlying message inline. That observability is what made this diagnosable in one round-trip.
  - **Follow-on: the normaliser alone wasn't enough.** `/locales?include_all=true` returns an object whose **keys are the locale codes and whose values are plain display-name strings** — `{ "en-us": "English - United States", … }`, not objects. `Object.values()` therefore produced an array of *names*, every `l.code` was `undefined`, and the picker filtered them all out ("Contentstack returned no locales."). Confirmed against v2's `AddStack`, which reads it as `Object.keys(res.data.locales).map(k => ({ value: k, label: locales[k] }))`. `listContentstackLocales` now uses `Object.entries` for the object form and keeps the array form as a fallback. My first test had *guessed* an object-of-objects shape; it was corrected to the real one — a case of a test encoding an unverified assumption, which only checking the prior art settled.
  - ⚠️ **The same latent bug exists in `cs-source-selection`'s reads** — `listOrgs`, `listStacks` and `listBranches` (`csManagement.service.ts:148/159/170`) all use the unguarded `(x ?? []).map`. Not changed here, as they belong to the sibling feature, but they will throw the same way if Contentstack ever returns those object-keyed.
