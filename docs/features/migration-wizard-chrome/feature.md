...
### FR — Copy
- **FR-5.1:** The primary action's label MUST be, per step: Source → "Proceed to audit"; Audit → "Continue to content mapping"; Destination → "Proceed to content mapping"; Content mapping → "Continue to preview"; Preview → "Start migration". Labels for Migrate and Verify are `TBD — Open Question` (Q-3).
